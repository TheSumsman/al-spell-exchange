"""Generate build/PolishSpellExchangeSheet.gs.

Some things simply do not survive an .xlsx import into Google Sheets:

  * data validation whose source is a range on ANOTHER sheet  (the Copy Planner
    character picker) -- silently dropped
  * checkboxes -- .xlsx has no such cell type at all
  * conditional formatting rules

Rather than hand-build the whole spreadsheet in Apps Script and lose the offline
formula verification, the workbook still ships as .xlsx (structure + 17k verified
formulas) and this short script applies the Sheets-only polish afterwards.

Run it against the converted sheet, once, after linking the form.

Note: the template below uses __TOKEN__ placeholders rather than %-formatting,
because the generated JavaScript is full of Logger.log('%s') calls that would
...
def main():
    ...
    with open(OUT, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(body)
    print("Wrote %s" % OUT)
    print("  roster %d, spell rows start at %d" % (N_WIZ, PLAN_TOP))
